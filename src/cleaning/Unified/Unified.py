...
class Unified ():
    def __init__ (self ,csv_d ,csv_c ,fd_path ,theta =0.01 ,beta =0.4 ):
        ...
        # Legacy implementation note.
        self .dirty_csv .replace (r'\t','',regex =True ,inplace =True )# Legacy implementation note.
        self .dirty_csv =self .dirty_csv .applymap (lambda x :x .strip ()if isinstance (x ,str )else x )# Legacy implementation note.
        ...
        # Legacy implementation note.
        self .clean_csv .replace (r'\t','',regex =True ,inplace =True )# Legacy implementation note.
        self .clean_csv =self .clean_csv .applymap (lambda x :x .strip ()if isinstance (x ,str )else x )# Legacy implementation note.
    ...
    def resolve_fd (self ,fd_path ):
        ...
        for line in lines :
            fd_this =[]
            line =line .split ("⇒",1 )
            left ,right =[i .strip ()for i in line [0 ].strip ().split (",")],[i .strip ()for i in line [1 ].strip ().split (",")]
            fd_this .append (left )
    ...
    def FIND_DATA_REPAIRS (self ,fd_idx ):
        ...
        for devi_v in tqdm (self .orifd_devi_pat [fd_idx ].keys (),ncols =90 ):
            ...
            for idx in impact_fds :
                ...
                cost_d +=self .pat_len_changed_cal (idx ,changed_core_pat ,self .orifd_core_pat )+self .pat_len_changed_cal (idx ,changed_devi_pat ,self .orifd_devi_pat )
                # Cal current fd change
            changed_fdcore_pat ,changed_fddevi_pat =self .com_single_core_devi (fd_idx ,self .tmp_csv [attrs_list ])
            delta_dl =self .pat_len_changed_cal (fd_idx ,changed_fdcore_pat ,self .orifd_core_pat )+self .pat_len_changed_cal (fd_idx ,changed_fddevi_pat ,self .orifd_devi_pat )
